...
class CouponService
{
    ...
    private function flatDiscount($planPrice)
    {
        if ($planPrice <= $this->coupon->discount_amount) {
            $this->discountAmount[$this->coupon->code] = formatCurrencyAmount($planPrice);
        }
        
        $this->discountAmount[$this->coupon->code] = formatCurrencyAmount($this->coupon->discount_amount);
    }
    ...
    private function percentageDiscount($planPrice)
    {
        ...
        if ($this->coupon->maximum_discount_amount > 0 && $discount > $this->coupon->maximum_discount_amount) {
            $this->discountAmount[$this->coupon->code] = formatCurrencyAmount($this->coupon->maximum_discount_amount);
        } else {
            $this->discountAmount[$this->coupon->code] = formatCurrencyAmount($discount);
        }
    }
    ...
    public function getDiscountAmount($planId = null, $userId = null, $billingCycle = null)
    {
        ...
        return $userDiscount > $planPrice ? formatCurrencyAmount($planPrice) : $userDiscount;
    }
