<form action="{{ route(config($addon->get('alias') . '.store_route')) }}" method="post">
    @csrf
    <div class="addon-modal-body">
        @php
            $fields = config($addon->get('alias') . '.fields');
        @endphp
                ...
                <div class="addon-modal-field">
                    @if ($field['type'] == 'text')
                        <input type="text"
                            ...
                            value="{{ config('openAI.is_demo') ? techEncrypt($value) : $value ?? '' }}">
