...
@section('logo', asset(config('iyzico.logo')))
@section('gateway', config('iyzico.name'))
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('iyzico.alias')])) }}" method="post"
        id="payment-form">
        @csrf
        <div>
            <div id="card-element">
