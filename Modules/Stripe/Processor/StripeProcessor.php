...
class StripeProcessor implements PaymentProcessorInterface, RequiresCallbackInterface
{
    ...
    private function stripeSetup($request)
    {
        try {
            ...
            $this->successUrl = route(config('gateway.payment_callback'), withOldQueryIntegrity(['gateway' => 'stripe']));
            $this->cancelUrl = route(config('gateway.payment_cancel'), withOldQueryIntegrity(['gateway' => 'stripe']));
