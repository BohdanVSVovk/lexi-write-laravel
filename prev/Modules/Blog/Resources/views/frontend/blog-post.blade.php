...
@section('content')
    <!-- component -->
    <div class="dark:bg-color-29 font-Figtree mt-[-10px] pt-[56px] min-h-[60vh]">
        @if (count($blogs) >= 1)
                ...
                <nav class="leading-4 md:mb-5 mb-6" aria-label="Breadcrumb">
                    <ol class="list-none inline-flex">
                        <li class="flex items-center">
                            ...
                            <a class="text-color-89 dark:text-color-DF font-medium md:text-14 text-12 ml-2" href="{{ URL::to('/') }}">{{ __('Home') }}</a>
                            <p class="text-color-89 dark:text-color-DF font-medium md:text-14 text-12 ml-2">/ </p>
                        </li>
                        <li>
                            @if (request()->search)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2" aria-current="page">{{ __('Search Results') }}</a>
                            @elseif (request()->year)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2" aria-current="page">{{ __('Archives') . ' ' . request()->year }}</a>
                            @elseif ($blogCategory)
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2" aria-current="page">{{ $blogCategory->name }}</a>
                            @else
                                <a href="#" class="text-color-2C dark:text-white md:text-14 text-12 font-medium ml-2" aria-current="page">{{ __('All Posts') }}</a>
                            @endif
                        </li>
                    </ol>
                </nav>
                    ...
                    <div class="md:flex justify-between w-full lg:p-0 lg:mt-4">
                        ...
                        <div class="md:w-4/6 lg:w-full">
                            @foreach ($blogs as $blog)
                                <div class="block lg:flex lg:mb-[50px] md:mr-[30px] lg:mr-0">
                                    <div class="rounded-md image-width-height blog-image-width-height overflow-hidden">
                                        <div class="grows">
                                            <a href="{{ route('blog.details', ['slug' => $blog->slug]) }}">
                                                <img class="h-60 w-[340px] image-width-height object-fit neg-transition-scale blog-image-width-height" src="{{ $blog->fileUrl() }}" alt="{{ $blog->title }}">
                                            </a>
                                        </div>
                                    </div>
                                    <div class="flex justify-start lg:w-[494px] w-full lg:mx-[30px] items-center">
                                        <div class="md:my-5 my-[18px]">
                                            <div>
                                                <p class="text-color-2C dark:text-white md:text-13 text-12 -mt-0.5 font-medium ml-1 mr-5">
                                            ...
                                            <div class="md:mt-[13px] mt-2.5">
                                                <a href="{{ route('blog.details', $blog->slug) }}"
                                                    ...
                                                    <svg class="ml-2 relative mt-1 w-3 h-2 md:w-4 md:h-4"
