            ...
            <div class="relative w-full">
                <form action="{{ route('blog.search') }}" method="get">
                    <div class="h-[50px] focus:border-color-2C lg:w-[270px] w-full rounded border text-decoration-none hover:border-gray-200 border-color-DF">
                        <input class="border-none text-14 h-12 w-full pr-12 text-color-89 dark:text-color-DF dark:bg-color-29 dark:rounded-xl mr-0.5" type="text" name="search" value="{{request()->search}}" placeholder="{{ __('Search your keyword') }}..">
                    </div>
                    <div class="absolute top-4 right-5">
                        <button type="submit">
                            <svg class="cursor-pointer text-color-2C dark:text-white" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
        ...
        @foreach ($popularBlogs as $data)
                <div class="flex mb-5 gap-3">
                    <a class="h-20 lg:w-20 w-28" href="{{ route('blog.details', ['slug' => $data->slug]) }}">
                        <img class="h-20 lg:w-20 w-28 rounded object-cover" src="{{ $data->fileUrl() }}" alt="{{ $data->title }}">
                    </a>
