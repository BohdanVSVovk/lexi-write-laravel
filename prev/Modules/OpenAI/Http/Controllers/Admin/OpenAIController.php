...
class OpenAIController extends Controller
{
    ...
    public function createContentPreferences(Request $request, ChatBot $chatBot)
    {
        ...
        $chatBot->store(['name' => $request->name]);

        $validator = Preference::aiSettingValidation($request->only('openai', 'stablediffusion', 'short_desc_length', 'long_desc_length', 'long_desc_length', 'ai_model', 'max_token_length'));

        if ($validator->fails()) {
            return back()->withErrors($validator)->withInput();
        }

        $post = $request->only('openai', 'stablediffusion', 'short_desc_length', 'long_desc_length', 'long_desc_length', 'ai_model', 'max_token_length');
        $request->file_id = '';
        $i = 0;
        $response=[];
