            ...
            <div class="flex justify-between items-center lg:mx-6 mb-4 xl:px-0 px-5 gap-5">
                <div>
                    <div class="flex gap-1.5 leading-[22px] items-center font-medium text-color-2C dark:text-white text-[15px] edit-url">
                        ...
                        <p class="content-name pl-3 line-clamp-single break-all">{{ __('Content of a :x', ['x' => trimWords($slug, 120) ?? '']) }}</p>
                    </div>
                </div>
                <div class="flex justify-end 2xl:gap-10 gap-3">
                    <p class="text-center word-counter text-color-2c dark:text-white whitespace-nowrap text-[15px] leading-[22px] font-medium">{{ __('Total Word') }} : {{ $useCase->words ?? 0 }} </p>
...
@section('js')
    ...
    <script src="{{ asset('Modules/OpenAI/Resources/assets/js/tiny_mce.js') }}"></script>
