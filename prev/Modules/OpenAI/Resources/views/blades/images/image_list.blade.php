...
<div class="dark:bg-[#292929] bg-[#F6F3F2] sidebar-scrollbar overflow-auto flex flex-col flex-1 font-Figtree border-l dark:border-[#474746] border-color-DF h-screen">
    <div class="9xl:px-[185px] 7xl:px-[140px] px-5 pt-[74px] 9xl:pb-[22px] pb-28">
        <div class="flex justify-between items-center mt-2.5 mb-5 gap-3">
            <a class="font-semibold text-color-14 dark:text-white text-xl" href="javascript: void(0)">
                {{ __('Image History') }}
            </a>
            <a class="items-center gap-2 text-color-2C dark:text-white text-right inline-block"
                href="{{ route('user.imageTemplate') }}">
                <span class="w-[14px] h-[14px] inline-block">
                    <svg class="w-[14px] h-[14px] mt-0.5" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14"
                    fill="none">
                ...
                <span class="font-medium text-15 break-words md:w-full text-right">{{ __('Create New Image') }}</span>
            </a>
        </div>
                ...
                <div class="xl:overflow-y-auto rounded-xl p-1.5">
                    <table class="min-w-full">
                        <thead class="bg-color-DF dark:bg-[#474746] rounded-xl">
                            <tr class="rounded-lg">
                                <th
                                    class="pl-3 lg:pl-[34px] py-[9px] text-left font-Figtree text-14 font-medium text-color-14 dark:text-white w-[224px]">
                                    {{ __('Image') }}
                                </th>
                                <th
                                    class="md:pl-[34px] md:pr-6 px-3 py-[9px] font-Figtree text-left text-14 font-medium text-color-14 w-[200px] dark:text-white break-words">
                                    {{ __('Prompt') }}
                                </th>
                                <th
                                ...
                                <th
                                    class="pr-3 xl:pr-[34px] py-[9px] text-right font-Figtree text-14 font-medium text-color-14 dark:text-white w-max">
                                    {{ __('Action') }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                                @forelse ($images as $image)
                                    <tr class="border-b dark:border-[#474746]" id="image_{{ $image->id }}">
                                        <td class="pl-3 lg:pl-[34px] 2xl:w-[224px] lg:w-[124px] flex py-5">
                                            <span class="justify-center w-[52px] h-[52px] lg:h-16 lg:w-16 flex items-center">
                                                <img class="rounded-md w-[52px] h-[52px] lg:w-16 lg:h-16 border border-color-DF dark:border-color-3A"
                                                    src="{{ $image->imageUrl() }}"
                                                    alt="">
                                            </span>
                                        </td>
                                        ...
                                        <td
                                            class="text-13 font-Figtree py-[22px] text-color-89 font-medium pr-3 xl:pr-[34px] w-max align-top xl:align-middle text-right">
                                            <div class="gap-4 justify-end items-center hidden xl:flex">
                                                <div class="relative">
                                                    <a class="image-tooltip-view flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center" title="{{ __('View Image')}}"  href="{{ route('user.image.view', ['slug' => $image->slug]) }}">
                                                ...
                                                <div class="relative">
                                                    <a class="image-tooltip-download flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center" title="{{ __('Download Image')}}"  href="{{ $image->imageUrl() }}" download="{{ $image->name }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                        height="16" viewBox="0 0 16 16" fill="none">
                                            ...
                                            <div class="relative xl:hidden inline-block">
                                                ...
                                                <div class="absolute right-0 mt-2 w-[201px] border border-color-89 dark:border-color-47 rounded-lg bg-white dark:bg-[#333332] z-50 table-drop-body dropdown-shadow">
                                                    <div>
                                                        <a href="{{ route('user.image.view', ['slug' => $image->slug]) }}" class="flex justify-start items-center gap-1.5 text-14 font-normal text-color-14 dark:text-white font-Figtree px-4 py-2 hover:bg-color-F6 dark:hover:bg-[#3A3A39] rounded-t-lg text-left">
                                                            <span class="w-4 h-4">
