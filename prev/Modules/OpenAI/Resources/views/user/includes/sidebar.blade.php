        ...
        <div class="mt-[22px] total-use-cases">
            <div>
                <div class="mb-3 custom-dropdown-arrow font-normal text-14 text-color-2C dark:text-white">
                    ...
                    <select
                        class="select block w-full text-base leading-6 font-medium text-color-FFR bg-white bg-clip-padding bg-no-repeat rounded-xl dark:bg-[#333332] dark:rounded-2xl m-0 focus:text-color-2C focus:bg-white focus:border-color-89 focus:outline-none use-cases break-words"
                        required="required">
                        @php $slug = $slug ?? ''; @endphp
                        @foreach ($useCases as $k => $data)
                            <option value="{{ $data->slug }}" {{ $data->slug == $slug ? 'selected' : '' }}>
                                {{ $data->name }}
                            </option>
                        @endforeach
                    </select>
