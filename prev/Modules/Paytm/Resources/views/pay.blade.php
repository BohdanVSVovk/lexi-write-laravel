...
@section('logo', asset(config('paytm.logo')))

@section('gateway', config('paytm.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('paytm.alias')])) }}" method="post"
        id="payment-form">
        @csrf
        <div>
            <div id="card-element">
