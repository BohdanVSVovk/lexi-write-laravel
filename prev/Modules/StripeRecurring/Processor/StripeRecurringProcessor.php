...
class StripeRecurringProcessor implements PaymentProcessorInterface
{
    ...
    {
        if (!$package?->sale_price || !$this->purchaseData?->currency_code || !$package?->billing_cycle) {
            throw new \Exception(__("Something is wrong with Stripe Recurring. Please try again."));
        }
        // Convert price to cents 
        $priceCents = round($package->sale_price * 100);
        $plan = \Stripe\Plan::create(array(
            ...
            "interval" => $this->planInterval[$package->billing_cycle],
