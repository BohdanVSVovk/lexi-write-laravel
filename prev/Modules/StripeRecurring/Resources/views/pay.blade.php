...
@section('logo', asset(config('striperecurring.logo')))

@section('gateway', config('striperecurring.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('striperecurring.alias')])) }}" method="post"
        id="payment-form">
        @csrf
        <div>
