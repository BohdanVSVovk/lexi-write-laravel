...
$(function () {
    ...
    function setTransaction(data) {
        if (data) {
            $('#billing_price, #amount_billed').val(data.discount_price > 0 ? data.discount_price : data.sale_price)
            $('#billing_cycle').val(data.billing_cycle)

            if ($('#payment_status').val() == 'Paid') {
                $('#amount_received').val(data.discount_price > 0 ? data.discount_price : data.sale_price)
                $('#amount_due').val(0)
            } else {
                $('#amount_due').val(data.discount_price > 0 ? data.discount_price : data.sale_price)
                $('#amount_received').val(0)
            }

            if (data.billing_cycle == 'days') {
                $('#duration_days').removeClass('d-none')
    ...
    function setDate(data) {
        ...
        if (data.billing_cycle == 'days') {
            ...
            day = billing_cycle[data.billing_cycle]
        }
