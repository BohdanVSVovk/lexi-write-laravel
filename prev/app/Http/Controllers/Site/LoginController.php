...
use App\Models\{User, PasswordReset, RoleUser, Role};
        ...
        if (Hash::check($data['password'], $userData->password)) {
            ...
            if (Auth::guard('user')->attempt($data)) {
                ...
                if (auth()->user()->role()->type == 'admin') {
                    return redirect()->route('dashboard')->withCookie($this->getUserThemePreferenceCookie());
                } else {
                    return redirect()->route('user.dashboard')->withCookie($this->getUserThemePreferenceCookie());
                }
            }
        ...
        if (!$request->isMethod('post')) {
            ...
            try {
                ...
                if (!empty($id)) {
                    ...
                    $emailResponse = $request['status'] === "Pending" ? (new UserVerificationCodeMailService)->send($request) : (new UserMailService)->send($request);
