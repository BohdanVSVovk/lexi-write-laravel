...
trait hasFiles
{
	...
	{
		return createDirectory(join(DIRECTORY_SEPARATOR, ['public', 'uploads', config('openAI.thumbnail_dir'), $size, $this->checkDirectory()]));
	}
    ...
    {
        $cacheKey = config('cache.prefix') . '.loadItem.' . $this->table . $this->id;
        $options = array_merge([
            'default' => true,
            'type' => $this->objectType()
        ], $options);
        ...
        if (cache()->has($cacheKey)) {
            return url($this->filePath(cache()->get($cacheKey)));
        } else {
            ...
            cache()->put([$cacheKey => $file->file_name], now()->addDays(1));
    ...
    public function imageUrl(array $options = []): string
    {
        ...
        $image = $this->where('id', $this->id)->first();
        ...
        if(is_null($image) || !file_exists($this->filePath('aiImages/'.$image->original_name)) || empty($image->original_name)) {
            return $this->defaultFileUrl($options['type']);
        }

        return url($this->filePath('aiImages/'.$image->original_name));
    }
}
