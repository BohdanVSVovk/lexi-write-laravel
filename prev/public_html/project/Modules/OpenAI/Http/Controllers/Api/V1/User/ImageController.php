...
class ImageController extends Controller
{
    ...
    {
        $configs        = $this->initialize([], request()->all());
        $images = $this->imageService->model()->where('user_id', auth('api')->user()->id)->orderBy("id", "desc")->groupBy('parent_id');
        if (count(request()->query()) > 0) {
            $images = $images->filter();
        }
    ...
    public function converstaionView($id)
    {
        ...
        $configs        = $this->initialize([], request()->all());
        $images = $this->imageService->model()->where(['user_id' => auth('api')->user()->id, 'parent_id' => $id])->orderBy("id", "desc");
        if (count(request()->query()) > 0) {
            $images = $images->filter();
        }
